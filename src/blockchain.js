...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      resolve(block);
      reject(new Error("Block Not Added"));
    });
  }
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let blockContent = { owner: address, star: star };

      if (currentTime - messageTime < 5 * 60 * 1000) {
        if (bitcoinMessage.verify(message, address, signature)) {
          let block = new BlockClass.Block(blockContent);
          await self._addBlock(block);
          resolve(block);
        } else {
          reject(new Error("error with submitting a star"));
        }
      }
    });
  }
  ...
  getBlockByHeight(height) {
    let self = this;
    return new Promise((resolve, reject) => {
      let sameHeigthBlocks = self.chain.filter(
        block => block.height === height
      )[0];
      if (sameHeigthBlocks) {
        resolve(sameHeigthBlocks);
      } else {
        resolve(null);
      }
    });
  ...
  validateChainAtBlock(block, perviousBlock, errorLogArray) {
    if (block.previousBlockHash !== perviousBlock.hash) {
      errorLogArray.push(`The blockchain is broken at block ${block.height}`);
    }
  }
}
